    ...
    <div class="container my-5">
        <div class="row set">
            ...
            <div class="col-md-6">
                ...
                <button class="btn btn-primary btn-lg pur ml-5" @click="purchase">购买</button>
...
async function showProduct(){
    ...
    try {
        ...
        const response = await axios.post('http://localhost/detail.php',
            { proId: proId.value },
            {
                headers: {
...
async function addCart(){
    if(userStore.isLoggedIn){
        if (quantity.value > 0 && quantity.value <= productInfo.value.stock)
            await carStore.addToCart(userStore.userId, proId, quantity.value)
        else {
            alert("请检查添加商品的数量是否合法")
        }
    }else{
        alert("请先登录后再添加购物车")
        router.push('/login')
